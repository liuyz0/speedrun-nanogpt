    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                v = state["exp_avg_sq"]
                # v <- beta2*v + (1-beta2)*g^2  (in-place, minimal allocs)
                v.mul_(beta2).add_((g * g).sum(dim=-1, keepdim=True), value=1.0 - beta2)

                update = m / (v.sqrt() / (1 - beta2 ** state['step']) ** 0.5 + eps)

                # p <- p - lr * update
                p.add_(update, alpha=-lr / (1 - beta1 ** state['step']))
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.embed.weight.detach().div_(self.embed.weight.data.norm(dim = -1, keepdim=True))
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        x = x0 = self.embed(input_seq)[None] * 768 ** 0.5 # use of norm here by @Grad62304977
...
value_embed_params = [p for n, p in model.named_parameters() if "value_embeds" in n]
embed_params = [p for n, p in model.named_parameters() if "embed" in n and "value" not in n]
...
optimizer1 = DistAdam(scalar_params + value_embed_params + head_params, lr=0.008, betas=(0.8, 0.95), eps=1e-10, weight_decay=0.0)
optimizer2 = Muon(hidden_matrix_params, lr=0.05, momentum=0.95, weight_decay=0.0)
optimizer3 = Roar(embed_params, lr=0.005, betas=(0.8, 0.95), eps=1e-10)
...
for step in range(train_steps + 1):
    ...
    # normalize embed
    #with torch.no_grad():
        #model.embed.weight.div_(model.embed.weight.norm(dim = -1, keepdim=True))
    # logging
